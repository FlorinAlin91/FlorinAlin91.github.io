(() => {
  ...
  const particles = [];
  const MAX = 260;
  const GOLD = [255, 180, 40];   // auriu
  const BLUE = [60, 140, 255];   // albastru
  const BLEND = 'lighter';

  function resize(){
    W = canvas.clientWidth; H = canvas.clientHeight = document.querySelector('.hero').clientHeight;
  ...
  function spawn(x, baseY, color){
    ...
    particles.push({x, y: baseY, vx:(Math.random()-0.5)*0.35, vy:-(1.2+Math.random()*1.6),
      life:0, maxLife:100+Math.random()*80, size:s, color});
  }
  ...
  function loop(){
    ...
    g.addColorStop(0,'#050506'); g.addColorStop(1,'#0a0f1a'); // negru -> albastru închis
    ...
    for(let i=particles.length-1;i>=0;i--){
      ...
      const c=[mix(p.color[0],255,k*0.6),mix(p.color[1],255,k*0.5),mix(p.color[2],255,k*0.6)];
      const alpha=0.08*(1-k);
      const r=p.size*(1+k*1.5)*2.2;
      const grd=ctx.createRadialGradient(p.x,p.y,0,p.x,p.y,r);
      grd.addColorStop(0, rgba(c,alpha)); grd.addColorStop(1, rgba(c,0));
